...
import { PackageSearchService } from '../../src/packageSearchService';
import { ArisPackageInstaller } from '../../src/arisPackageInstaller';

suite('Aris Package Manager Compatibility Suite', () => {
  let searchService: PackageSearchService;
  let searchServiceAny: any;
  let originalGetArisIndexPackages: any;
  let originalSearchGitHub: any;

  setup(() => {
    searchService = PackageSearchService.getInstance();
    searchService.clearCache();
    searchServiceAny = searchService as any;
...
suite('Aris Package Installer Helpers Suite', () => {
  let installerAny: any;

  setup(() => {
    installerAny = new ArisPackageInstaller() as any;
  });
